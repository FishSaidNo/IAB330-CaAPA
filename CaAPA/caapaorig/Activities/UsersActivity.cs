...
namespace caapa.Activities
{
    ...
    public class UsersActivity : Activity
    {
        ...
        private MobileServiceClient Users;

        //Mobile Service sync table used to access data
        private IMobileServiceSyncTable<Users> usersTable;

        //Adapter to map the items list to the view
        private Adapters.UsersAdapter adapter;
        ...
        protected override async void OnCreate (Bundle bundle)
        {
           ...
           // await InitLocalStoreAsync();
            ...
            var usersTable = client.GetSyncTable <Users> ();

            //textNewToDo = FindViewById<EditText> (Resource.Id.textNewToDo); //change to fit

            // Create an adapter to bind the items with the view
            adapter = new Adapters.UsersAdapter(this, Resource.Layout.Row_List_To_Do);
            var listViewUsers = FindViewById<ListView> (Resource.Id.listViewToDo);
        ...
        public async Task SyncAsync()
        {
			try {
	            ...
	            await usersTable.PullAsync("User", usersTable.CreateQuery()); // query ID is used for incremental sync
       ...
       public async void OnRefreshItemsSelected ()
        ...
        public async Task RefreshItemsFromTableAsync ()
        {
            try {
                ...
                var list = await usersTable.Where (user => user.Complete == false).ToListAsync ();

                adapter.Clear ();

                foreach (Users current in list)
                    adapter.Add (current);
