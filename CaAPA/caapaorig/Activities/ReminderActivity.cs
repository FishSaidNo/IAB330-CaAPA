...
namespace caapa.Activities
{
    ...
    public class ReminderActivity : Activity
    {
        ...
        private MobileServiceClient Reminder;

        //Mobile Service sync table used to access data
        private IMobileServiceSyncTable<Reminder> reminderTable;

        //Adapter to map the items list to the view
        private Adapters.ReminderAdapter adapter;
        ...
        protected override async void OnCreate (Bundle bundle)
        {
           ...
           // await InitLocalStoreAsync();
            ...
            var promptTable = client.GetSyncTable <Reminder> ();

            //textNewToDo = FindViewById<EditText> (Resource.Id.textNewToDo); //change to fit

            // Create an adapter to bind the items with the view
            adapter = new Adapters.ReminderAdapter(this, Resource.Layout.Row_List_To_Do);
            var listViewReminder = FindViewById<ListView> (Resource.Id.listViewToDo);
        ...
        public async Task SyncAsync()
        {
			try {
	            ...
	            await reminderTable.PullAsync("Reminders", reminderTable.CreateQuery()); // query ID is used for incremental sync
       ...
       public async void OnRefreshItemsSelected ()
        ...
        public async Task RefreshItemsFromTableAsync ()
        {
            try {
                ...
                var list = await reminderTable.Where (reminder => reminder.Complete == false).ToListAsync ();

                adapter.Clear ();

                foreach (Reminder current in list)
                    adapter.Add (current);
        ...
        [Java.Interop.Export()]
        public async void AddReminder(View view) {
            ...
            try {
                await reminderTable.InsertAsync(reminder); // insert the new item into the local database
                await SyncAsync(); // send changes to the mobile service
